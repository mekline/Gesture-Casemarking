...
def session(me):
	...
	log('willowSubNo','Paycode','movieCheck', 'nounTrainingTime', 'verbTrainingTime', 'refreshTrainingTime', 'trialNo',
		...
		'Response', 'wasCorrect','wasError', 'wordResponseA', 'wordResponseB', 'testSequence')
	...
	for i in xrange(len(items)):
		...
		testWordList = myVerbs + myNouns
		randState = random.getstate()
		fixedShuffle(testWordList,randState)

		####
		#Start display - Hand out number list
		####

		toScreen("GM_math_prep.html", [("aaa", testWordList[0]), ("bbb", testWordList[1]),("ccc", testWordList[2]),("ddd", testWordList[3]),("eee", testWordList[4])])
		...
		for j in range(5):
			...
			myResp = myResp.lower()
			wordAnswers.append(myResp)

		wordResponseCorrectA = 0
		for j in range(5):
			if wordAnswers[j] == testWordList[j].lower().replace(' ',''):
				wordResponseCorrectA = wordResponseCorrectA + 1
		...
		wordResponseCorrectB = 0
		for j in range(5):
			if wordAnswers[j] == testWordList[j].lower().replace(' ',''):
				wordResponseCorrectB = wordResponseCorrectB + 1

	 	####
	 	#Show feedback screen
	 	####
	 	
	 	if (wasCorrect & (wordResponseCorrectA + wordResponseCorrectB==10)):
	 		toScreen("progress_feedback.html",[("XXX", "RIGHT"),("BBB", "right"),("YYY",str(wordResponseCorrectA + wordResponseCorrectB)),("ZZZ", str(i+1)),("PPP", str(100*(i+1)/len(items))),("AAA", str(len(items)))])
	 	elif wasCorrect:
	 		toScreen("progress_feedback.html",[("XXX", "WRONG"),("BBB", "right"),("YYY",str(wordResponseCorrectA + wordResponseCorrectB)),("ZZZ", str(i+1)),("PPP", str(100*(i+1)/len(items))),("AAA", str(len(items)))])
		else:
			toScreen("progress_feedback.html",[("XXX", "WRONG"),("BBB", "wrong"),("YYY",str(wordResponseCorrectA + wordResponseCorrectB)),("ZZZ", str(i+1)),("PPP", str(100*(i+1)/len(items))),("AAA", str(len(items)))])
		...
		log(me, payCode, movieCheck, nounTrainingTime, verbTrainingTime, refreshTrainingTime, i, stimNo[i], sentType[i],
			...
			wasCorrect,wasError, wordResponseCorrectA, wordResponseCorrectB, TestSequence)
